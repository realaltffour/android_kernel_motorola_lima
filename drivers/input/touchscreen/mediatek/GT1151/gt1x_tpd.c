...
/*1 enable,0 disable,touch_panel_eint default status, need to confirm after
 * register eint*/
int irq_flag = 1;
...
static irqreturn_t tpd_eint_interrupt_handler(unsigned irq,
...
static s32 i2c_dma_read_mtk(u16 addr, u8 *buffer, s32 len) {
	...
	},
	{
	    .flags = I2C_M_RD,
	    .ext_flag =
		(gt1x_i2c_client->ext_flag | I2C_ENEXT_FLAG | I2C_DMA_FLAG),
	    .addr = (gt1x_i2c_client->addr & I2C_MASK_FLAG),
	    .timing = I2C_MASTER_CLOCK,
	    .buf = (u8 *)(uintptr_t)gpDMABuf_pa,
	},
...
#ifdef CONFIG_MTK_I2C_EXTENSION
	    .addr =
		((gt1x_i2c_client->addr & I2C_MASK_FLAG) | (I2C_ENEXT_FLAG)),
	    .timing = I2C_MASTER_CLOCK,
#else
...
static int tpd_power_on(void) {
    ...
    if (gt1x_get_chip_type() != 0) return -1;

    if (gt1x_reset_guitar() != 0) return -1;
...
void gt1x_power_switch(s32 state) {
	...
	case SWITCH_ON:
		...
		if (ret) GTP_ERROR("regulator_enable() failed!\n");
#else
#ifdef TPD_POWER_SOURCE_CUSTOM
#ifdef CONFIG_ARCH_MT6580
		ret = regulator_set_voltage(tpd->reg, 2800000,
					    2800000); /*set 2.8v*/
		if (ret) GTP_DEBUG("regulator_set_voltage() failed!\n");
		ret = regulator_enable(tpd->reg); /*enable regulator*/
		if (ret) GTP_DEBUG("regulator_enable() failed!\n");
#else
	...
	case SWITCH_OFF:
		...
		if (ret) GTP_ERROR("regulator_disable() failed!\n");
#else
#ifdef TPD_POWER_SOURCE_CUSTOM
#ifdef CONFIG_ARCH_MT6580
		ret = regulator_disable(tpd->reg); /*disable regulator*/
		if (ret) GTP_DEBUG("regulator_disable() failed!\n");
#else
...
int gt1x_is_tpd_halt(void) { return tpd_halt; }
...
int gt1x_pm_notifier(struct notifier_block *nb, unsigned long val, void *ign) {
	...
	case PM_RESTORE_PREPARE:
		...
		/* pr_err("%s: stoping update thread(%d)", __FUNCTION__,
...
static int tpd_registration(void *client) {
    ...
    if (gt1x_init()) {
	/* TP resolution == LCD resolution, no need to match resolution when
	 * initialized fail */
	gt1x_abs_x_max = 0;
	gt1x_abs_y_max = 0;
    }
    ...
    update_thread =
	kthread_run(gt1x_auto_update_proc, (void *)NULL, "gt1x_auto_update");
...
void gt1x_touch_down(s32 x, s32 y, s32 size, s32 id) {
    ...
    if (ret) GTP_ERROR("Touch down get lcm_x failed");
    ret = kstrtoul(CONFIG_CUSTOM_LCM_Y, 0, &lcm_y);
    if (ret) GTP_ERROR("Touch down get lcm_y failed");
    ...
    //#ifdef CONFIG_MTK_BOOT
    //	if (tpd_dts_data.use_tpd_button) {
    //		if (FACTORY_BOOT == get_boot_mode() || RECOVERY_BOOT ==
    // get_boot_mode()) 			tpd_button(x, y, 1);
    //	}
    //#endif
}
...
void gt1x_touch_up(s32 id) {
...
#endif
    TPD_DEBUG_SET_TIME;
    TPD_EM_PRINT(tpd_history_x, tpd_history_y, tpd_history_x, tpd_history_y, id,
		 0);
    tpd_history_x = 0;
    tpd_history_y = 0;
    //#ifdef CONFIG_MTK_BOOT
    //	if (tpd_dts_data.use_tpd_button) {
    //		if (FACTORY_BOOT == get_boot_mode() || RECOVERY_BOOT ==
    // get_boot_mode()) 			tpd_button(0, 0, 0);
    //	}
    //#endif
}
...
static int tpd_event_handler(void *unused) {
	...
	if (finger == 0x00) gt1x_request_event_handler();
...
int gt1x_debug_proc(u8 *buf, int count) {
	...
	free_irq(touch_irq, NULL);
	ret = tpd_irq_registration();
	if (ret < 0) GTP_ERROR("rerequest_irq fail, %d!", ret);
...
static int tpd_local_init(void) {
    ...
    if (IS_ERR(tpd->reg)) GTP_ERROR("regulator_get() failed!\n");
#endif
#endif
#if TPD_SUPPORT_I2C_DMA
    tpd->dev->dev.coherent_dma_mask = DMA_BIT_MASK(32);
    gpDMABuf_va = (u8 *)dma_alloc_coherent(
	&tpd->dev->dev, IIC_DMA_MAX_TRANSFER_SIZE, &gpDMABuf_pa, GFP_KERNEL);
    if (!gpDMABuf_va) {
...
#endif
#endif
    if (is_resetting || update_info.status) return;
...
static void tpd_resume(struct device *h) {
    ...
    if (ret < 0) GTP_ERROR("GTP later resume failed.");
#ifdef CONFIG_GTP_HOTKNOT
    if (!hotknot_enabled) gt1x_send_cmd(GTP_CMD_HN_EXIT_SLAVE, 0);
#endif
...
void tpd_on(void) {
    ...
    while (retry++ < 5) {
	ret = tpd_power_on();
	if (ret < 0) GTP_ERROR("I2C Power on ERROR!");
